...
export class ThemeExpansionService {
    ...
    for (const theme of themes) {
      ...
      if (theme.sourceThemes && theme.sourceThemes.length > 1) {
        ...
        const exceedsLineLimit =
          strictAtomicLimits && totalLines > maxAtomicSize;
        ...
        if (exceedsLineLimit || exceedsFileLimit) {
          ...
          if (expansionCandidate) {
            ...
            console.log(
              `[RE-EVALUATION] Merged theme "${theme.name}" remains atomic despite size`
            );
   ...
   */
  private validateThemeCodeContext(theme: ConsolidatedTheme): boolean {
    // Check if codeContext exists
    if (!theme.codeContext || !theme.codeContext.files) {
      console.warn(
        `[CONTEXT-VALIDATION] Theme "${theme.name}" missing codeContext`
      );
      return false;
    }
    ...
    if (contextFiles.size !== claimedFiles.size) {
      console.warn(
        `[CONTEXT-VALIDATION] File count mismatch for theme "${theme.name}": context=${contextFiles.size}, claimed=${claimedFiles.size}`
      );
      return false;
    }
    ...
    for (const file of claimedFiles) {
      if (!contextFiles.has(file)) {
        console.warn(
          `[CONTEXT-VALIDATION] Theme "${theme.name}" claims file "${file}" but no changes found in codeContext`
        );
        return false;
      }
    }
    ...
    const hasValidChanges = theme.codeContext.files.some(
      (file) =>
    ...
    if (!hasValidChanges) {
      console.warn(
        `[CONTEXT-VALIDATION] Theme "${theme.name}" has no valid code changes`
      );
      return false;
    }

    console.log(
      `[CONTEXT-VALIDATION] Theme "${theme.name}" passed validation: ${contextFiles.size} files, ${theme.codeContext.totalLinesChanged} lines`
    );
    return true;
  }
  ...
  private async deduplicateSubThemes(
    ...
    subThemes.forEach((t) => {
      ...
      console.log(
        `  - "${t.name}" (${t.affectedFiles.length} files, ${lines} lines)`
      );
    });
    ...
    if (
      ...
      secondPassResult.forEach((t) => {
        ...
        if (t.sourceThemes && t.sourceThemes.length > 1) {
          console.log(
            `  - "${t.name}" (MERGED from ${t.sourceThemes.length} themes, ${t.affectedFiles.length} files, ${lines} lines)`
          );
        } else {
          console.log(
            `  - "${t.name}" (unchanged, ${t.affectedFiles.length} files, ${lines} lines)`
          );
        }
      });
...
Return JSON with specific sub-themes:
{
  ...
  private extractSubThemeCodeContext(
    ...
    const totalLines = filteredFiles.reduce(
      (sum, file) =>
        ...
        file.changes.reduce(
          (fileSum, change) =>
            fileSum + (change.endLine - change.startLine + 1),
          0
        ),
      0
    );
   ...
   */
  private async verifyAtomicLeaves(
    themes: ConsolidatedTheme[],
    currentDepth: number
  ): Promise<void> {
    for (const theme of themes) {
      ...
      if (theme.childThemes.length === 0) {
        ...
        if (!atomicCheck.isAtomic) {
          console.warn(
            `[ATOMIC-VIOLATION] Leaf theme "${theme.name}" at depth ${currentDepth} is not atomic`
          );
          console.warn(`[ATOMIC-VIOLATION] Reason: ${atomicCheck.reason}`);
          console.warn(
            `[ATOMIC-VIOLATION] Files: ${theme.codeContext.files.length}, Lines: ${theme.codeContext.totalLinesChanged}`
          );
          ...
          console.log(
            `[ATOMIC-VERIFICATION] ✅ Leaf theme "${theme.name}" is properly atomic`
          );
        }
      } else {
   ...
   */
  private async verifyAtomicTheme(theme: ConsolidatedTheme): Promise<{
    isAtomic: boolean;
    reason: string;
...

{
  ...
  "hasNaturalBoundary": boolean
    ...
    try {
      ...
      if (extractionResult.success) {
        const result = extractionResult.data as {
          isAtomic?: boolean;
          reasoning?: string;
          singleResponsibility?: boolean;
          unitTestable?: boolean;
        ...
        console.warn(
          `[ATOMIC-VERIFICATION] JSON parsing failed: ${extractionResult.error}`
        );
        console.warn(
          `[ATOMIC-VERIFICATION] Raw response: ${response.substring(0, 500)}`
        );
